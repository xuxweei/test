...
<div class="node" :class="{ selected: data.selected }" :style="nodeStyles" data-testid="node">
  ...
  <!-- Controls-->
  <div class="control" v-for="[key, control] in controls" :key="key" :ref="el => onRef(el, key, control, 'control')" :data-testid="'control-'+key"></div>
  <!-- Inputs-->
  <div class="input" v-for="[key, input] in inputs" :key="key" :data-testid="'input-'+key">
    ...
    <div class="input-control" v-show="input.control && input.showControl" :ref="el => onRef(el, key, input.control, 'control')" data-testid="input-control"></div>
  </div>
</div>
</template>
